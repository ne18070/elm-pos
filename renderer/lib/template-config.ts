...
export function createTemplate(businessId: string, base?: Partial<TemplateConfig>): TemplateConfig {
  ...
  const tpl: TemplateConfig = { ...DEFAULT_THERMAL, ...base, id };
  const existing = getTemplates(businessId);
  saveTemplates(businessId, [...existing, tpl]);
  return tpl;
}

// ─── Helpers (duplicated from invoice-templates.ts to avoid circular deps) ───
...
function buildBody(order: any, business: any, config: TemplateConfig, extra?: { resellerName?: string; resellerClientName?: string; resellerClientPhone?: string }): string {
  ...
  if (isThermal) {
    return `
      ...
      <div class="center" style="margin-bottom:8px">
        ${config.showLogo && business.logo_url ? `<img src="${business.logo_url}" style="max-width:60px;max-height:40px;margin-bottom:4px"><br>` : ''}
        <div class="bold big">${business.name}</div>
        ${config.showAddress && business.address ? `<div class="small">${business.address}</div>` : ''}
        ${config.showPhone && business.phone ? `<div class="small">Tél : ${business.phone}</div>` : ''}
        ${config.showEmail && business.email ? `<div class="small">${business.email}</div>` : ''}
      ...
      <!-- Paiements -->
      ${config.showPaymentDetails ? `<table>
        ...
        ${config.showChange && change > 0 ? `<tr><td>Rendu monnaie</td><td style="text-align:right">${fmt(change, cur)}</td></tr>` : ''}
        ${config.showBalance && remaining > 0.01 ? `<tr style="color:#e53e3e"><td>Reste à payer</td><td style="text-align:right">${fmt(remaining, cur)}</td></tr>` : ''}
      </table>` : ''}
      ...
      <div class="center small" style="margin-top:4px">
        ${config.footerText || (business.receipt_footer ?? 'Merci de votre visite !')}
      </div>
  ...
  return `
    ...
    <div class="header">
      <div class="biz-info">
        ${config.showLogo && business.logo_url ? `<img src="${business.logo_url}" class="logo">` : ''}
        <div>
          <div class="biz-name">${business.name}</div>
          ${config.showAddress && business.address ? `<div class="biz-detail">${business.address}</div>` : ''}
          ${config.showPhone && business.phone ? `<div class="biz-detail">Tél : ${business.phone}</div>` : ''}
          ${config.showEmail && business.email ? `<div class="biz-detail">${business.email}</div>` : ''}
      ...
      <div class="invoice-meta">
        ...
        ${config.showReceiptNum ? `<div class="invoice-num">${receiptNum(order)}</div>` : ''}
    ...
    <div class="totals">
      <table class="totals-table">
        ...
        ${config.showBalance && remaining > 0.01 ? `<tr style="color:#c0392b;font-weight:600"><td>Reste dû</td><td>${fmt(remaining, cur)}</td></tr>` : ''}
        ${config.showChange && change > 0 ? `<tr><td>Rendu monnaie</td><td>${fmt(change, cur)}</td></tr>` : ''}
      </table>
    </div>
    ...
    ${config.footerText || business.receipt_footer ? `<div class="footer-text">${config.footerText || business.receipt_footer}</div>` : ''}
  `;
}
