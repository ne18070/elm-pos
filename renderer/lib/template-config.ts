...
function renderCustomText(block: TemplateBlock, isThermal: boolean): string {
  ...
  return `<div style="text-align:${align};${bold}font-size:${size};margin:6px 0;color:#000">${content}</div>`;
}
  ...
  return `
    <div class="totals" data-section="totals">
      <table class="totals-table">
        ...
        ${config.showBalance && ctx.remaining > 0.01 ? `<tr style="color:#000;font-weight:600"><td>Reste dû</td><td>${fmt(ctx.remaining, cur)}</td></tr>` : ''}
...
function renderPayment(ctx: RenderCtx, isThermal: boolean): string {
  ...
  return `
    <table data-section="payment">
      ...
      ${config.showBalance && ctx.remaining > 0.01 ? `<tr style="font-weight:600"><td>Reste à payer</td><td style="text-align:right">${fmt(ctx.remaining, cur)}</td></tr>` : ''}
    </table>
    ${loyaltyHtml}
    ${notesHtml}`;
}
...
function renderFooter(ctx: RenderCtx, isThermal: boolean): string {
  ...
  if (isThermal) {
    return `
      ...
      <div class="center small" style="margin-top:6px;color:#000">
      ...
      <div class="center" style="margin-top:8px">
        ...
        <div class="small" style="margin-top:2px;color:#000">${receiptNum(order)}</div>
      </div>` : ''}`;
  }
  // A4
...
function buildBody(
  ...
  const itemsHtml = ((order.items ?? []) as any[]).map((item: any) => {
    if (isThermal) {
      return `
        ...
        <tr>
          ${config.showUnitPrice ? `<td style="padding-left:8px;color:#000">${item.quantity} × ${fmt(item.price, cur)}</td>` : `<td style="padding-left:8px;color:#000">Qté : ${item.quantity}</td>`}
          <td style="text-align:right;font-weight:600">${fmt(item.total, cur)}</td>
        </tr>
        ${config.showItemDiscount && item.discount_amount > 0 ? `<tr><td colspan="2" style="padding-left:8px;color:#000;font-size:10px">Remise: -${fmt(item.discount_amount, cur)}</td></tr>` : ''}
        ${config.showItemNotes && item.notes ? `<tr><td colspan="2" style="padding-left:8px;font-size:10px;color:#000;font-style:italic">${item.notes}</td></tr>` : ''}`;
    }
    return `
      <tr>
  ...
  const couponLine = config.showCoupon
    ? (order.coupon_code
      ? (isThermal
        ? `<tr><td>Coupon (${order.coupon_code})</td><td style="text-align:right">-${fmt(order.discount_amount, cur)}</td></tr>`
        : `<tr><td>Coupon (${order.coupon_code})</td><td>-${fmt(order.discount_amount, cur)}</td></tr>`)
      : order.discount_amount > 0
      ? (isThermal
        ? `<tr><td>Remise</td><td style="text-align:right">-${fmt(order.discount_amount, cur)}</td></tr>`
        : `<tr><td>Remise</td><td>-${fmt(order.discount_amount, cur)}</td></tr>`)
      : '')
    : '';
...
function a4Css(config: TemplateConfig, isTwoCols: boolean): string {
  ...
  return `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .biz-detail { font-size: 9px; color: #000; line-height: 1.4; }
    .invoice-meta { text-align: right; }
    .invoice-title { font-size: 16px; font-weight: 800; letter-spacing: .05em; color: #000; }
    .invoice-num { font-size: 12px; font-weight: 700; color: #000; }
    .invoice-detail { font-size: 9px; color: #000; }
    .parties-row { display: flex; justify-content: space-between; gap: 12px; margin: 8px 0; }
    .party-box { flex: 1; border: 1px solid #000; border-radius: 4px; padding: 6px 10px; font-size: 10px; }
    .party-left { border-left: 3px solid #000; }
    .party-right { border-right: 3px solid #000; text-align: right; }
    .party-label { font-size: 8px; text-transform: uppercase; letter-spacing: .05em; color: #000; margin-bottom: 2px; }
    ...
    .note { font-size: 9px; color: #000; font-style: italic; }
...
export function renderTemplate(
  ...
  if (isThermal) {
    ...
    const label1 = config.showCopyLabel ? `\n      <div class="copy-label">${config.copy1Label}</div>` : '';
    const label2 = config.showCopyLabel ? `\n      <div class="copy-label">${config.copy2Label}</div>` : '';
