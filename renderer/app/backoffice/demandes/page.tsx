  ...
  return (
    <div className="p-8 space-y-8 pb-32">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight uppercase">Demandes Clients</h1>
          <p className="text-slate-500 text-sm mt-1">Gérez les inscriptions et les validations de paiement.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="card p-6 border-l-4 border-l-amber-500 bg-amber-500/5">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">En attente</p>
        ...
        <div className="card p-6 border-l-4 border-l-blue-500 bg-blue-500/5">
          ...
          <p className="text-3xl font-black text-white mt-1">{allRequests.length}</p>
        </div>
      </div>
                  ...
                  return (
                    <tr key={req.id} className={cn("hover:bg-surface-hover/30 transition-colors", req.status !== 'pending' && "opacity-50")}>
                      <td className="px-6 py-4">
                        <p className="font-black text-white leading-tight">{req.business_name}</p>
      ...
      >
        <div className="space-y-6">
          <div>
            <label className="label text-[10px] font-black uppercase tracking-widest text-slate-500">Choix du Plan</label>
            ...
            <div>
               ...
               <div className="flex bg-surface-input rounded-xl p-1 h-12">
                  {(['jours', 'mois'] as const).map(m => (
                    <button key={m} onClick={() => setApproveForm((f: any) => ({ ...f, mode: m }))} className={cn("flex-1 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all", approveForm?.mode === m ? "bg-brand-600 text-white shadow-lg" : "text-slate-500 hover:text-slate-300")}>{m}</button>
                  ))}
               </div>
            </div>
          </div>
          <div>
            <label className="label text-[10px] font-black uppercase tracking-widest text-slate-500">Note interne (Optionnel)</label>
        ...
        footer={
          <div className="flex gap-4">
            ...
            <button onClick={onApprovePublic} disabled={!!processing} className="btn-primary flex-1 h-12 font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 shadow-xl shadow-brand-500/20">
              {processing && <Loader2 size={16} className="animate-spin" />} Créer & Envoyer Acces
            </button>
          </div>
        }
      >
        <div className="space-y-6">
          <div className="p-5 rounded-2xl bg-brand-500/5 border border-brand-500/10">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Prospect</p>
            <p className="text-sm font-bold text-white">{approvePublicForm?.req.email}</p>
          ...
          <div>
            <label className="label text-[10px] font-black uppercase tracking-widest text-slate-500">Plan</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
               ...
               <div className="flex bg-surface-input rounded-xl p-1 h-12">
                  {(['jours', 'mois'] as const).map(m => (
                    <button key={m} onClick={() => setApprovePublicForm((f: any) => ({ ...f, mode: m }))} className={cn("flex-1 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all", approvePublicForm?.mode === m ? "bg-brand-600 text-white shadow-lg" : "text-slate-500 hover:text-slate-300")}>{m}</button>
                  ))}
               </div>
            </div>
          </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="label text-[10px] font-black uppercase tracking-widest text-slate-500">Motif du rejet (Optionnel)</label>
