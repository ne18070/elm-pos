...
import { Loader2, Save, UserPlus, Shield, UserX, ChevronDown, Users, User } from 'lucide-react';
...
export default function AdminPage() {
  const { user, business, setUser } = useAuthStore();
  ...
  async function handleAvatarChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const url = await uploadProductImage(user.id, file); // réutilise storage
  ...
  async function handleRemove(member: UserType) {
    ...
    try {
      await removeUserFromBusiness(member.id);
          ...
          <div className="max-w-2xl space-y-4">
                ...
                {members.map((member) => {
                  ...
                  return (
                    <div key={member.id} className="card p-4 flex items-center gap-4">
                      {/* Avatar lettre */}
                      <div className="w-10 h-10 rounded-xl bg-surface-input border border-surface-border flex items-center justify-center text-base font-bold text-brand-400 shrink-0">
                        {member.avatar_url ? (
            ...
            <div className="card p-4 space-y-2 mt-4">
              ...
              <div className="space-y-1.5 text-xs text-slate-400">
                <p><span className="text-yellow-400 font-medium">Propriétaire</span> — Accès total, gestion du business et de l'équipe</p>
