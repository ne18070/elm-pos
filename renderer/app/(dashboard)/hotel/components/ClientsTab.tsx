        ...
        {filteredGuests.map((g) => (
          <div key={g.id} className="card p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-content-primary truncate">{g.full_name}</p>
              {g.phone && <p className="text-xs text-content-secondary flex items-center gap-1 mt-0.5"><Phone className="w-3 h-3" />{g.phone}</p>}
              {g.nationality && <p className="text-xs text-content-primary">{g.nationality}</p>}
              {g.id_number && <p className="text-xs text-content-primary">{g.id_type} {g.id_number}</p>}
            </div>
            <div className="flex gap-1 shrink-0">
              <button onClick={() => openGuestPanel(g)} className="p-1.5 rounded-lg text-content-secondary hover:text-content-primary hover:bg-surface-hover">
