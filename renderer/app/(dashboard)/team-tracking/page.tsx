...
import { MapPin, Navigation, Clock, User, Globe } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { fr } from 'date-fns/locale';
import { cn } from '@/lib/utils';

export default function TeamTrackingPage() {
  const { terminals } = useRealtimeStore();
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1.5 flex items-center gap-1">
                        ...
                        Actif {formatDistanceToNow(new Date(member.joined_at), { addSuffix: true, locale: fr })}
                      </p>
                    </div>
                  </div>
                ...
                <div key={member.terminal_id} className="p-4 flex items-center justify-between hover:bg-surface-hover/30 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-slate-400 font-medium">
                      {formatDistanceToNow(new Date(member.joined_at), { addSuffix: true, locale: fr })}
                    </p>
                  </div>
                </div>
              ))}
