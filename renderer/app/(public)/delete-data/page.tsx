    ...
    <div className="theme-dark min-h-screen bg-surface flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-badge-orange flex items-center justify-center mx-auto mb-4">
            <svg className="w-7 h-7 text-status-orange" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
          <h1 className="text-2xl font-black text-content-primary">Suppression de données</h1>
        ...
        {step === 'form' && (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-surface-card rounded-2xl border border-surface-border/30 p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-surface-input border border-surface-border rounded-xl px-4 py-3 text-content-primary placeholder-content-muted text-sm focus:outline-none focus:border-status-orange/50 transition-colors"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-content-secondary uppercase tracking-widest mb-3">
                  Données à supprimer *
                </label>
                    ...
                    <label
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                        selected.includes(dt.id)
                          ? 'border-status-orange/50 bg-badge-orange'
                          : 'border-surface-border/30 bg-surface-input/30 hover:bg-surface-input'
                      }`}
                    >
                      <input
                        type="checkbox"
                      ...
                      <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${
                        selected.includes(dt.id) ? 'bg-orange-500 border-orange-500' : 'border-surface-border'
                      }`}>
                        {selected.includes(dt.id) && (
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-content-primary">{dt.label}</p>
                        <p className="text-[11px] text-content-muted">{dt.description}</p>
                      </div>
                    </label>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-surface-input border border-surface-border rounded-xl px-4 py-3 text-content-primary placeholder-content-muted text-sm focus:outline-none focus:border-status-orange/50 transition-colors resize-none"
                />
              </div>
            </div>
            ...
            <p className="text-center text-xs text-content-muted">
              ...
              <a href="/delete-account" className="text-content-secondary underline">cliquez ici</a>
            </p>
          </form>
        )}
        ...
        {step === 'done' && (
          <div className="bg-surface-card border border-surface-border/30 rounded-2xl p-8 text-center space-y-3">
            ...
            <p className="text-sm text-content-secondary">
              Les données sélectionnées seront supprimées dans un délai de <strong className="text-content-primary">30 jours</strong>. Votre compte reste actif.
            </p>
            <p className="text-xs text-content-muted mt-2">
              Confirmation envoyée à <span className="text-content-secondary">{email}</span>
            </p>
          </div>
        )}
