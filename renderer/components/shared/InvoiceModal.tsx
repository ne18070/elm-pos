...
import { X, Printer, Eye, Settings2, FileText } from 'lucide-react';
import type { DistributeurInvoiceType } from '@/lib/invoice-templates';
...
const FORMAT_BADGE: Record<TemplateConfig['format'], string> = {
  ...
  'a5-portrait':     'A5',
  'a4-distributeur': 'A4 Distributeur',
};
...
export function InvoiceModal({
  ...
  const [docType, setDocType] = useState<DistributeurInvoiceType>('FACTURE');
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div className="bg-surface-card border border-surface-border rounded-2xl w-full max-w-lg shadow-2xl flex flex-col">
          ...
          <div className="p-5 space-y-3">
            ...
            {/* Type document — uniquement pour le modèle Distributeur */}
            {isDistributeur && (
                  ...
                  {(['FACTURE', 'BON DE FACTURE'] as DistributeurInvoiceType[]).map((t) => (
                    <button
                      ...
                      className={`py-2.5 px-3 rounded-xl border text-sm font-semibold transition-all ${
                        docType === t
                          ? 'border-brand-600 bg-brand-600/10 text-content-brand'
