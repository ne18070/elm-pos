    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card p-4 text-center">
          <p className="text-xs text-content-secondary mb-1">CA Grossiste</p>
          <p className="text-lg font-bold text-status-warning">{loading ? '…' : fmt(totalCA)}</p>
        </div>
        <div className="card p-4 text-center">
        ...
        <div className="card p-4 text-center">
          ...
          <p className="text-lg font-bold text-status-info">{loading ? '…' : totalClients}</p>
        </div>
      </div>
      ...
      <div className="card p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          {/* Client filter */}
          <div className="relative">
            <Users className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-content-secondary pointer-events-none" />
            <select
          ...
          {/* Product filter */}
          <div className="relative">
            <Package className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-content-secondary pointer-events-none" />
            <input
              type="text"
              placeholder="Produit…"
              value={productSearch}
        ...
        {hasFilter && (
          <button
            ...
            className="text-xs text-content-brand hover:text-content-brand flex items-center gap-1"
          >
            ...
            return (
              <div key={r.reseller_id} className="card overflow-hidden">
                ...
                >
                  ...
                  <p className="text-sm font-bold text-status-warning shrink-0 mr-2">{fmt(r.revenue)}</p>
                  {isExpanded
                ...
                {isExpanded && (
                  <div className="border-t border-surface-border divide-y divide-surface-border">
                    ...
                    {/* Products */}
                    <div className="px-4 py-3">
                      <p className="text-xs font-semibold text-content-secondary mb-3 flex items-center gap-1.5">
                            ...
                            <div key={p.name} className="flex items-center gap-2">
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between text-xs mb-0.5">
                                  <span className="text-content-primary truncate">{p.name}</span>
                                  <span className="text-status-warning shrink-0 ml-2">{fmt(p.revenue)}</span>
                                </div>
