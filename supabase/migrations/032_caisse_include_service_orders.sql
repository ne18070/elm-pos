-- Migration 032 : Inclure les paiements service_orders dans la caisse
-- Les OT payés (status = 'paye') sont désormais comptabilisés dans
-- get_session_live_summary et close_cash_session, au même titre que
-- les ventes boutique et les paiements hôtel.
...
BEGIN
  ...
  UPDATE public.cash_sessions SET
    ...
    expected_cash = v_session.opening_amount + v_cash + v_h_cash + v_s_cash,
    actual_cash   = p_actual_cash,
    difference    = p_actual_cash - (v_session.opening_amount + v_cash + v_h_cash + v_s_cash),
    notes         = p_notes
